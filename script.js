...
/* ── 6. BUY NOW BUTTONS ─────────────────────────────────────── */
/**
 * Avocado Oil products use data-product attributes resolved here.
 * Beef Tallow products have Stripe links hardcoded directly as href.
 *
 * Avocado Oil Stripe links:
 */
const STRIPE_LINKS = {
...
function initBuyButtons() {
  document.querySelectorAll('.btn--buy').forEach(btn => {
    ...
    // Only intercept data-product buttons that need a link resolved.
    // Buttons with a direct href (beef tallow) are already live.
    if (product && link) {
      btn.href   = link;
      btn.target = '_blank';
      btn.rel    = 'noopener';
    } else if (product && !link && !btn.href.startsWith('http')) {
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        alert(`Checkout link coming soon for "${product}". Check back shortly.`);
      });
    }
  });
}
